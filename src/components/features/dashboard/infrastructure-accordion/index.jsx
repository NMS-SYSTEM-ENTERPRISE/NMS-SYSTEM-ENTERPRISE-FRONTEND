    ...
    <div className={styles.latencyViz}>
      <div className={styles.latencyStrip}>
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
            className={styles.latencyBlock}
            style={{
              backgroundColor: i < blocks ? color : 'var(--color-bg-tertiary)',
            }}
          />
        ))}
      </div>
            ...
            {currentDevices.map((device, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td>
                    <UptimeVisual uptime={device.uptime} />
                  </td>
                  <td>
                    <LatencyBar ping={device.ping} />
                  </td>
                  <td style={{ textAlign: 'center' }}>
                    <span
...
export const InfrastructureAccordion = ({ statistics }) => {
  ...
  const tabs = [
    {
      ...
      data: statistics.total_devices || [],
    },
    {
      id: 'online',
      label: 'Online',
      count: statistics.online || 0,
      data: statistics.online_devices || [],
    },
    {
      id: 'offline',
      label: 'Offline',
      count: statistics.offline || 0,
      data: statistics.offline_devices || [],
    },
  ];
  ...
  const availableTypes = [
    'All',
    ...Array.from(new Set(currentTabData.map((d) => d.type).filter(Boolean))),
  ];
                      ...
                      {availableTypes.map((type) => {
                        const typeCount =
                          type === 'All'
                            ...
                            : currentTabData.filter((d) => d.type === type)
                                .length;
                        const typeVis =
                          type === 'All'
                            ? {
