...
export const downloadPngExport = async ({ screenTitle, dashboardElement, previewElement }) => {
  // Use dashboard element if available, otherwise fall back to preview
  const elementToCapture = dashboardElement || previewElement;

  if (!elementToCapture) {
    throw new Error('Dashboard element or preview element is required for PNG export');
  }
